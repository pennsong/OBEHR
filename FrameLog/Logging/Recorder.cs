...
namespace FrameLog.Logging
{
        ...
        where TChangeSet : IChangeSet<TPrincipal>
    {
        ...
        public void Record(object entity, string reference, string propertyName, Func<object> deferredValue)
        {
            ensureChangeSetExists();

            string typeName = ObjectContext.GetObjectType(entity.GetType()).Name;
            string key = reference;
            var objectChange = getObjectChangeFor(set, typeName, key);
            record(objectChange, propertyName, deferredValue);
        }
        private void record(IObjectChange<TPrincipal> objectChange, string propertyName, Func<object> deferredValue)
        {
            IPropertyChange<TPrincipal> propertyChange = getPropertyChangeFor(objectChange, propertyName);
            if (deferredValue != null)
            {
                deferredValues.Store(objectChange, propertyName, deferredValue);
            }
        }
        ...
        private IObjectChange<TPrincipal> getObjectChangeFor(TChangeSet set, string typeName, string key)
        {
